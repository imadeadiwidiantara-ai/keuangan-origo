...
-- --- FUNCTION: tandai_bukti_transaksi ---
-- Dipakai kasir untuk menandai struk sudah dicetak / invoice sudah
-- dikirim, tanpa perlu izin UPDATE penuh di tabel transaksi (yang
-- sengaja tidak diberikan supaya kasir tidak bisa ubah harga/status).
...
-- --- MASTER DATA & PENGATURAN ---create policy "master_terapis_select" on master_terapis for select using (true);
